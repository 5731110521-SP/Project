...
public class Winner extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		if (winner == 1 || winner == 2) {
			...
			g2.drawString(Login.player[winner - 1].getName() + " is the winner.",
					GameScreen.width / 2 - (int) r2.getWidth() / 2, 270);
			g2.setFont(new Font("Tahoma", 0, 20));
			fm = g.getFontMetrics();
			r2 = fm.getStringBounds("Level UP! " + Login.player[winner - 1].getLevel() + " > "
					+ (Login.player[winner - 1].getLevel() + 1), g);
			g2.drawString(
					"Level UP! " + Login.player[winner - 1].getLevel() + " > "
							+ (Login.player[winner - 1].getLevel() + 1),
					GameScreen.width / 2 - (int) r2.getWidth() / 2, 300);
		} else {
			FontMetrics fm = g.getFontMetrics();
